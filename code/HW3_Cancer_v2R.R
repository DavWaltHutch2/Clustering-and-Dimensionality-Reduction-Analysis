...
data.x <- data[,!(colnames(data) %in% c("diagnosis"))]
...
## K-MEANS
## TODO: WHAT WAS MY MEASURE OF DISTANCE
##########################################################
...
for (i in 2:15)
{
  model.kmeans <- kmeans(data.input,centers=i,nstart = 20)
...
##CREATE MODEL
centers <- 3 ##IDEAL NUMBER OF CENTERS
start_time <- Sys.time()
model.kmeans <- kmeans(data.input,centers = centers, nstart = 1)
end_time <- Sys.time()
...
colvar[colvar == 1] <- NA
colvar[colvar == 2] <- 1
colvar[is.na(colvar)] <- 2
scatter3D(data.input[,7], data.input[,8],data.input[,6], colvar = colvar, theta = 0, phi = 20, 
...
models <- c("VVV")
start_time <- Sys.time()
model.em <- Mclust(data.x, modelNames = models)
end_time <-Sys.time()
total_time <- end_time - start_time
summary(model.em)
...
scatter3D(data.x[,7], data.x[,8],data.x[,6], col = c("dark blue","dark red"), theta = 0, phi = 20, 
...
plot(cummulative.importance, type = "b", xlab = "Principal Components", ylab = "Cumulative Proportion of Variance",
     main = "Cumulative Porportion of Variance")
 
##PLOT EIGEN VALUES
barplot(model.pca$sdev^2, xlab = "Principal Component", ylab = "Eigenvalue", main = "Eigenvalue Distribution")

##PLOT 3D
colvar <- as.numeric(data$diagnosis)
col <- as.character(data$diagnosis)
col[col == "B"] <- "blue"
col[col == "M"] <- "red"
...
scatter3D(model.pca$x[,28], model.pca$x[,29], model.pca$x[,30], colvar = colvar, theta = 0, phi = 20, 
          main = "Plot of Best Principle Components (PC28, PC29, PC30)")


##GET DATA
data.pca <- model.pca$x[,1:11]
str(data.pca)
...
corr <- cor(data.x)
corrplot(corr, method = "circle", tl.cex = .01)
...
col[col == "B"] <- "blue"
col[col == "M"] <- "red"
scatter3D(S[,1], S[,4], S[,3], colvar = colvar, theta = 0, phi = 20, 
          main = "High Kurtosis Independent Components") ##HIGH KURTOSIS


plot(S[,8],S[,2], col = col, xlab = "Independent Componennt 4", ylab = "Independent Component 11", 
     main = "Low Kurtosis Independent Components")


##GET DATA 
data.ica <- ica.model$S
str(data.ica)
data.ica.highK <- data.ica[,-c(8,2)]
str(data.ica.highK)
data.ica.lowK <- data.ica[,c(8,2)]
str(data.ica.lowK)
...
scatter3D(data.transformed[,1], data.transformed[,2], data.transformed[,3], colvar = colvar, 
          theta = 0, phi = 20, main = "Plot Over 3 Random Projections") 
...
colvar[colvar == 2] <- 1
colvar[is.na(colvar)] <- 2
scatter3D(data.reduced[,1], data.reduced[,2],data.reduced[,3], colvar = colvar, theta = 0, phi = 20, 
          main = "PCA and K-MEANS")
...
colvar[colvar == 2] <- 1
colvar[is.na(colvar)] <- 2
scatter3D(data.reduced[,7], data.reduced[,9],data.reduced[,5], colvar = colvar, theta = 0, phi = 20, 
          main = "ICA (HIGH KURTOSIS) and K-MEANS")
...
colvar[colvar == 2] <- 1
colvar[is.na(colvar)] <- 2
scatter2D(data.reduced[,1], data.reduced[,2], colvar = colvar, theta = 0, 
          main = "ICA (LOW KURTOSIS) and K-MEANS")
...
colvar[colvar == 2] <- 1
colvar[is.na(colvar)] <- 2
scatter3D(data.reduced[,1], data.reduced[,2], data.reduced[,2], colvar = colvar, theta = 0, phi = 20, 
          main = "RP and K-MEANS")
...
for (i in 2:15)
{
  model.kmeans <- kmeans(data.reduced,centers=i,nstart = 20, iter.max = 15)
  wss[i] <- sum(model.kmeans$withinss)
  sil[i] <- summary(silhouette(model.kmeans$cluster,distance))$avg.width
}
...
centers <- 3 ##IDEAL NUMBER OF CENTERS
start_time <- Sys.time()
model.kmeans <- kmeans(data.reduced,centers = centers, nstart = 1, iter.max = 15)
end_time <- Sys.time()
total_time <- end_time - start_time
clusters.kmeans <- model.kmeans$cluster
...
##GET MODEL
data.reduced <- as.data.frame(data.ica.lowK)
str(data.reduced)
model.em <- Mclust(data.reduced)
summary(model.em)
##plot(model.em) ##UNCOMMENT TO PLOT
...
##PLOT 3D - KMEANS
colvar <- as.numeric(clusters.em)
scatter2D(data.reduced[,1], data.reduced[,2], colvar = colvar, 
          main = "ICA (LOW KURTOSIS) and K-MEANS")
...
##GET DATA
#############################
data.reduced <- cbind(as.data.frame(data.ica.highK), "diagnosis" = data.y)
str(data.reduced)
...
##GET DATA
#############################
data.reduced <- cbind(as.data.frame(data.ica.lowK), "diagnosis" = data.y)
str(data.reduced)
